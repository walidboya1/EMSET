    ...
    if($row = mysqli_fetch_array($qrydisplay))
    {
        ...
        if ($pname1 <> "")
        {
            for ($i = 0;$i < $filecount;$i++)
            {
                ...
                clearStoredResults();
                $qrydisplay1 = mysqli_query($connection, $sel);
                if($row1 = mysqli_fetch_array($qrydisplay1))
                {
                $Cadre = $row1['id'];
                $pname = $_FILES['file']['name'][$i];
                #sql query to insert into database
                $sql = "INSERT into cours(URL,Cadre,filiere) values ('$pname','$Cadre','$filiere') ";
                clearStoredResults();
                ...
                move_uploaded_file($_FILES['file']['tmp_name'][$i], '../cours/' . $pname);
